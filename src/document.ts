...
export default class Document {
	private static doInsert(doc: vscode.TextDocument, editor: vscode.TextEditor, content: string): Thenable<void> {
		return editor.edit((builder) => {
			...
			doc.save();
			console.log('teratail edit done');
		}, (error) => {
			console.error('teratail edit failed: ');
			console.error(error);
			vscode.window.showErrorMessage('teratail failed to initialize. Please run command \'Reload Window\' to fix it.');
		});
	}
	...
	static openDocument(filename: string, content: string): Thenable<void> {
		console.log('teratail buffer file: ' + filename);
		return vscode.workspace.openTextDocument(filename).then((doc) => {
			console.log('teratail doc opened');
			return vscode.window.showTextDocument(doc).then((editor) => {
				console.log('teratail edit begins');
